...
import { setStoreForTests } from "../store";
import { memoryStore, resetMemoryStore } from "../store/memory";
...
describe.each(["memory", "postgres"] as const)("banco: %s", (backend) => {
  ...
  beforeEach(async () => {
    ...
    if (backend === "memory") {
      ...
      pg ??= new PGlite();
      await pg.exec("DROP TABLE IF EXISTS docs");
      const client = {
        query: async (text: string, params?: unknown[]) => {
          const r = await pg!.query<Record<string, unknown>>(text, params as never);
          return { rows: r.rows, rowCount: r.affectedRows ?? 0 };
        },
        release: () => {},
      };
      setStoreForTests(postgresStore({ connect: async () => client }, { singleConnection: true }));
    }
  });
